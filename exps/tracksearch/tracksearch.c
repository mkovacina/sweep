...
void scan(agent_struct *agent, int action_index){
  ...
  float radius = 2.5;  /*Radius of search area */
...
void offset(int *row, int *column){
  ...
  float theta;   /* Angle variable, range 0 to 2 pi */ 

  float radius;  /* Radius variable, range 0 to amount */

  float amount = 7;  /* Maximum offset amount */

  theta = ((2 * PI) * ((double)rand()/(double)RAND_MAX));
  radius  = ((amount) * ((double)rand()/(double)RAND_MAX)); 
  
  row_off = (int)(radius * sin(theta));
  col_off = (int)(radius * cos(theta));
