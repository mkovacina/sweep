	...
	/*  Generates a firefly "flash" within a limited area of the grid */
{
	...
	TraceDebug("rows = %d, cols = %d", rows, cols);
	...
	for (int row = Yloc-1; row < Yloc+2; row++)
	{
		for  (int col = Xloc-1; col < Xloc+2; col++)
		{
			...
			{
				int oldValue = GCURR(action_grid, row, col);
				GSET_CUM(action_grid, row, col, 1);
				int newValue = GCURR(action_grid, row, col);
				// don't forget the grids are FLOATS, not ints
				TraceDebug("[r,c]=[%d,%d]  %d => %d", row, col, oldValue, newValue);
			}
		}
	}
}
